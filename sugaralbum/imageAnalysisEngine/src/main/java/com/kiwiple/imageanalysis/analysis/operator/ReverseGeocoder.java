...
import com.kiwiple.imageanalysis.utils.BlobCache;
import com.kiwiple.imageanalysis.utils.CacheManager;

/**
 * 위도 경도를 주소 값으로 반환하는 클래스
 */
public class ReverseGeocoder {
    ...
    public ReverseGeocoder(Context context) {
        ...
        mGeoCache = CacheManager.getBlobCache(context, GEO_CACHE_FILE, GEO_CACHE_MAX_ENTRIES,
    ...
    private String computeAddress(SetLatLong set) {
    ...
    private Address lookupAddress(final double latitude, final double longitude, boolean useCache) {
        try {
    ...
    /**
     * 위도 경도 값의 주소 String을 반환
     * 
     * @param latitude 위도
     * @param longitude 경도
     * @return 주소
     */
    public String generateName(String latitude, String longitude) {
        ReverseGeocoder.SetLatLong set = new ReverseGeocoder.SetLatLong();
        double itemLatitude = Double.valueOf(latitude);
        double itemLongitude = Double.valueOf(longitude);
